...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("middleware.log"), logging.StreamHandler()],
)
...
async def process_webhooks(lottery_payload, power_automate_payload):
    ...
    try:
        lottery_result, power_automate_result = await asyncio.gather(
            *tasks, return_exceptions=True
        )

        results["lottery_system"] = (
            lottery_result
            if not isinstance(lottery_result, Exception)
            else {"success": False, "error": str(lottery_result)}
        )
        results["power_automate"] = (
            power_automate_result
            if not isinstance(power_automate_result, Exception)
            else {"success": False, "error": str(power_automate_result)}
        )
